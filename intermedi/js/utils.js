...
function comhohefetmultipleform(elementinici, elementfi) {
  let correccio_array = new Array();
  for (let i = elementinici; i <= elementfi; i++) {
    let resposta;
    resposta = eval("document." + nomform + ".resposta" + i + ".value");
    resposta = parsejaapostrof(resposta);
...
function comhohefetradioSP() {
  let correccio_array_radio = new Array();
  let resposta_radio = "0";
  nelements_radio.forEach(i => {
    ...
    let com_correccio_radio = false;
    let resultat_radio = false;
...
function comhohefetradio(
  ...
  let correccio_array_radio = new Array();
  let resposta_radio = "0";
  for (i = 0; i < nelements_radio; i++) {
    for (j = 0; j < possibilitats_radio; j++) {
      if (
        eval(
          "document." +
          nomform_radio +
    ...
    let com_correccio_radio = false;
    let contestada = false;
    com_correccio_radio = eval("'" + resposta_radio + "'=='" + correccio_array_radio + "'",
    );
    if (eval(com_correccio_radio)) {
      possibilitats_radio.forEach(j => {
        if (eval("document." + nomform_radio + ".resposta_radio" + i + "[" + j + "].checked",)) {
          eval("document." + nomform_radio + ".resposta_radio" + i + "[" + j + "].id='correct_radio'",);
          elem = document.getElementById(nomform_radio + "rr" + i + j);
          elem.src = "../../../../images/b_correcte.gif";
...
function comhohefetcheckbox() {
  let correccio_array_check = new Array();
  nelements_check.forEach(i => {
    possibilitats_check.forEach(j => {
      let resposta_check;
      eval("document." + nomform_check + ".resposta_check" + i + "[" + j + "].checked",) ? resposta_check = "1" : resposta_check = "0";
      correccio_array_check = correccio_check[i][j];
      let com_correccio_check = false;
...
function comhohefetText(correccioText, nelementsText, nomformText) {
  let correccio_array = new Array();
  let resposta_array = new Array();
  let resposta = "";
  let com_correccio = false;
  let resultat = false;
  let respostaUnitaria = "";
  let correccioUnitaria = "";

  nelementsText.forEach(i => {
    resposta = "";
    resposta = eval(
      "document." + nomformText + ".resposta" + i + ".id='normal2'",
    );
...
function netejarformText(nelementsText, nomformText) {
  nelementsText.forEach(i => {
...
function tornarescriurecheckbox(campform, i, j) {
  if (
    ...
    eval(
      "document." + nomform_check + "." + campform + "[" + j + "].checked=true",
    );
  }
  eval(
    "document." +
...
function tornarescriureText(campformText, i, nomformText) {
